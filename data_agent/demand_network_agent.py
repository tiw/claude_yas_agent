...
class DemandNetworkAgentConfig(BaseAgentConfig):
    """需求网络分析Agent配置"""
    pass


class DemandNetworkAnalysisAgent(BaseAgent):
    """需求网络分析Agent，专门用于市场需求分析和趋势预测"""
    
    def __init__(self, config: DemandNetworkAgentConfig):
        ...
        for server in default_servers.values():
            if server.server_type == ServerType.DEMAND_NETWORK:
    ...
    async def process_query(self, user_input: str, user_id: str = "anonymous", files: list = None) -> str:
        ...
        # 记录访问日志
        self.security.log_access(user_id, "process_query", "demand_network_analysis")
        
        with self.observability.tracer.start_as_current_span("process_query"):
            self.observability.record_query("demand_network_analysis")
    ...
    async def _parse_query(self, user_input: str) -> Dict[str, Any]:
        ...
        parse_prompt = self.prompt_manager.get_prompt("demand_network_query_parser")
    ...
    async def _generate_final_response(self, parsed_query: Dict[str, Any], results: List[Dict]) -> str:
        ...
        response_prompt = self.prompt_manager.get_prompt("demand_network_response_generator")
